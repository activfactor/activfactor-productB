...
export default (strategyResults) => {
    ...
    if (!isEmpty(strategyResults)){
        const {strategyName, benchmarkName, strategy: {actual, historical: strategyHistorical}, benchmark: {historical: benchmarlHistorical}} = strategyResults;
        if (actual){
            ...
            if (factorIntensity){
                const fethcedData = Object.keys(factorIntensity).map(factor => ({
                    data: [{y: factorIntensity[factor], name:factor}], name: factor
                }))
            ...
            if(strategyHistorical && benchmarlHistorical){
                ...
                if (strategyCumulativePerformance && benchmarkCumulativePerformance){
                    ...
                    data.cumulativePerformance = {
                        data: [{name: 'This strategy',data:strategyCumulative},{name: benchmarkName, data:benchmarkCumulative}],
                ...
                if (strategyAnnualReturn && benchmarkAnnualReturn){
                    ...
                    data.annualReturn = {
                        data: [{name: 'This strategy', data:strategyAnnual},{name: benchmarkName, data:benchmarkAnnual}],
                ...
                if (strategyDrawdown && benchmarkDrawdown){
                    ...
                    data.drawdown = {
                        data: [{name: 'This strategy', data:strategyDrawDownData},{name: benchmarkName, data:benchmarkDrawDownData}],
